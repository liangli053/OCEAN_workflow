...
class MPEntry:
  ...
  def  __init__(self, MP_ID, ctr_atom):
     ...
     self.formula = MP().get_data(MP_ID,"vasp",  "pretty_formula")[0]['pretty_formula']
     self.band_gap = MP().query(MP_ID, ['band_gap'])[0]['band_gap']
     self.total_formula = MP().query(MP_ID, ['unit_cell_formula'])[0]['unit_cell_formula']
     self.SP_symbol = MP().query(MP_ID, ['spacegroup'])[0]['spacegroup']['symbol']
     self.SP_number = MP().query(MP_ID, ['spacegroup'])[0]['spacegroup']['number']
     self.znucl = []
     self.typat = []
     self.pp_list = []
     ...
     self.get_pp_list()
     self.get_ngkpt()
     self.edge_info = Find_unique_sites(self.MP_ID,self.ctr_atom)
     self.computed_SP_number = self.edge_info.symm_finder.get_space_group_number()
     self.computed_SP_symbol = self.edge_info.symm_finder.get_space_group_symbol()
     self.print_spacegroup_info()

  def print_spacegroup_info(self):
    print("spacegroup of {} is: {:3d}  {:7s}\n".format(self.formula, self.SP_number, self.SP_symbol))
    print("spacegroup of Materials Project computed cell is: {:3d}  {:7s}\n".format(self.computed_SP_number, self.computed_SP_symbol))
    if self.SP_number is not self.computed_SP_number:
  ...
  def get_pp_list(self):
    for i in self.elements:
      tmp = Element(i)
      self.pp_list.append(str('{:02d}'.format(tmp.Z))+'-'+i.lower()+".lda.fhi")
...
#  def get_nkpt(self):
  ...
  def write_to_file(self, fout, ctr_atom):
    ...
    fout.write("# number of element types\n"
              ...
              "znucl  { "+" ".join(str(e) for e in self.znucl)+' }\n'
              "# number of total atoms\n"
              "natom  "+str(self.natom)+"\n"
              "pp_list  { "+"  ".join(self.pp_list)+' }\n'
